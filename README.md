...
<img src="assets/readme-hero.svg" width="100%" alt="君子经济学家：从经济问题与理论制度出发，以证据约束结构模型，完成反事实与福利分析">
...
| 创造、笃行、工具纪律与实践检验 | 结构建模、证据识别、反事实、福利与经济写作 |
...
| **术** | 怎样形成可信研究？ | 建立结构模型，以事实和简约式证据识别、约束和检验，完成估计、反事实与回溯 |
...
面向机制、均衡、福利和政策反事实的应用研究沿着下面的主线推进：

```text
经济问题 → 理论机制与制度环境 → 结构模型与政策目标 → 事实和简约式证据 → 结构识别与估计 → 反事实与福利
```

- **结构模型组织研究主线。** 它把主体、约束、信息、互动、动态和均衡连接起来，并明确最终要回答的政策与福利问题。
- **简约式证据约束结构。** 它用于发现事实、区分机制、识别行为边际或参数组合、构造估计目标，并检验模型未直接拟合的含义。
- **结构估计整合证据。** 参数、均衡响应、福利和样本外反事实必须继承数据支持、识别假设与模型设定的不确定性。
- **结构偏好不构成结果保证。** 数据无法约束可信模型时，应缩小、部分识别、重新调查或放弃当前结构分支。
...
| 理论与现实 | 证据与方法 | 成果与治理 |
|---|---|---|
| 微观、宏观、政治经济学与历史 | 结构建模、数据测量、识别、估计与验证 | 研究主线、主张账本、分支决策 |
...
```text
$junzi $junzi-economist 请把这个现实现象重建为结构经济学问题，提出模型、识别对象与反事实目标，并说明哪些事实和简约式证据能够约束和检验它。
```
